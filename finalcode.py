    ...
    while attempts < max_attempts:
        ...
        try:
            if category == "a":  # if choose animals open animal txt file
                file = open(file_path()+"\\words\\animal_words.txt","r")  # Open the file in read mode
                try:
                ...
                file = open(file_path()+"\\words\\car_words.txt","r")  # Open the file in read mode
                try:
                    for line in file:
                        word = line.strip()
                        words_.append(word)
                ...
                file = open(file_path()+"\\words\\cities_words.txt","r")  # Open the file in read mode
                try:
                    for line in file:
                        word = line.strip()
                        words_.append(word)
...
def save_game_data(words, word_positions, name, score, grid):
    filename = open(file_path()+"\\words\\saved_game.txt", "w")
...
def file_path():
    filename="C:\\Users\\Dell\\Desktop\\cr"
    return filename



def load_data():
    filename = file_path()+"\\words\\saved_game.txt"
...
def check_oldusername(username):
    filename = open(file_path()+"\\words\\saved_game.txt", "r")
    lines = filename.readlines()  # Read all lines into a list
    if len(lines) > 3:  # Ensure there are at least 3 lines to access line 2
        second_line = lines[2].strip()  # Get the second line (index 1) and remove extra whitespace
...
def main():
    ...
    if check_oldusername(name):
        ...
        if resume_game.lower() == "y":
            ...
            print("Deleted last saved game.Starting a new game...")
