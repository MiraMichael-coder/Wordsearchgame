...
/words/
  ...
  cities_words.txt
  saved_game.txt
Populate the word files with appropriate words (one per line)
...
Choose to resume a saved game (if available) or start new

Select difficulty level (1-3)

Choose a word category (a-c)

The game displays the word list and letter grid
...
Continue until all words are found or attempts run out
...
Modify file_path() to point to your working directory

Edit word list text files to change vocabulary

Adjust grid sizes in the level selection logic

Change attempt counter in main() for different challenge levels
